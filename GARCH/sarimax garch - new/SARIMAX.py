...
def fit_SARIMAX(gwap_endog, lwap_endog, train_exog, gwap_order, gwap_seasonal_order, GWAP_test, lwap_order, lwap_seasonal_order, LWAP_test, exog_test):
    ...
    return gwap_model_results, lwap_model_results, gwap_test_predict, lwap_test_predict, gwap_train_predict, lwap_train_predict

#Define a function to calculate MAE and RMSE
def evaluate_models(GWAP, LWAP, GWAP_test_inversed, GWAP_predictions_inverse, LWAP_test_inversed, LWAP_predictions_inverse, Region, Garch: str):
    ...
    ax.set_title(f'SARIMAX {Garch.upper()} GWAP {Region} Predictions')
    ax.legend()
    ...
    ax.set_title(f'SARIMAX {Garch.upper()} {Region} LWAP Predictions')
    ax.legend()
    plt.show()

#Define function to apply inverse transformations
def inverse_predictions(gwap_predictions, lwap_predictions, boxcoxy_fit_loaded, transformed_exog):
    # Combine predictions into one dataset
    all_predictions = pd.concat([gwap_predictions, lwap_predictions, transformed_exog], axis=1)
    
    # Access the scaler within the pipeline
    scaler = boxcoxy_fit_loaded.named_steps['minmax_scaler']  # Adjust 'scaler' to your step name
    
    # Apply inverse transformations
    all_predictions_inverse = scaler.inverse_transform(all_predictions)
    ...
    return GWAP_predictions_inverse, 
