...
def fit_SARIMAX(gwap_endog, lwap_endog, train_exog, gwap_order, gwap_seasonal_order, GWAP_train, GWAP_test, lwap_order, lwap_seasonal_order, LWAP_train, LWAP_test, exog_test):
    ...
    gwap_train_predict = gwap_model_results.predict(start=GWAP_train.index[0], end=GWAP_train.index[-1], exog=train_exog)
    ...
    # Predict on train set
    lwap_train_predict = lwap_model_results.predict(start=LWAP_train.index[0], end=LWAP_train.index[-1], exog=train_exog)
    fig, ax = plt.subplots(figsize=(7, 3))
    ...
    lwap_test_predict = appended_gwap.predict(start=LWAP_test.index[0], end=LWAP_test.index[-1], exog=exog_test, dynamic=False)
...
def evaluate_models(GWAP, LWAP, GWAP_test_inversed, GWAP_predictions_inverse, LWAP_test_inversed, LWAP_predictions_inverse, Region):
    ...
    #Calculate MSE on the test set
    lwap_mse_test = mean_squared_error(GWAP_test_inversed, GWAP_predictions_inverse)
